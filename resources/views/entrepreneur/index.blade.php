                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-900/50 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 font-semibold">Servicio / Negocio</th>
                                <th scope="col" class="px-6 py-4 font-semibold">Ciudad</th>
                                <th scope="col" class="px-6 py-4 font-semibold">Precio Referencia</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                    <td class="px-6 py-4">
                                        @if($service->image_path)
                                            <img src="{{ asset('storage/' . $service->image_path) }}" alt="Foto de {{ $service->title }}" class="w-16 h-16 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-600">
                                        @else
                                    ...
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                        {{ $service->title }}
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Tel: {{ $service->contact_phone }}</p>
                                    </td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300">
                                        {{ $service->city->name ?? 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-gray-600 dark:text-gray-300 font-medium">
                                        {{ $service->price ? 'C$ ' . number_format($service->price, 2) : 'A convenir' }}
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-right space-x-3">
                                        <a href="{{ route('entrepreneur.services.edit', $service->id) }}" class="inline-block text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300 font-medium transition-colors">
                                            Editar
                                        </a>
                                        
                                        <form action="{{ route('entrepreneur.services.destroy', $service->id) }}" method="POST" class="inline-block" id="form-delete-{{ $service->id }}" onsubmit="confirmarEliminacion(event, {{ $service->id }})">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        No tiene servicios turísticos o comerciales registrados por el momento.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        function confirmarEliminacion(event, id) {
            ...
            Swal.fire({
                ...
                text: "Esta acción borrará su servicio de forma permanente.",
                icon: 'warning',
